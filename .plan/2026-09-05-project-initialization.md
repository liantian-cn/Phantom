...
- Each condition instance uses exactly one output mode (`cell`, `value_bar`, or `icon_tile`) but may reserve multiple consecutive regions of that same mode. Its output contract separately declares `output_type`, `output_count`, `value_type`, and `value_shape`; the output count is computed after argument validation and remains fixed after layout. Multiple regions may combine into one scalar or decode into a list. For example, one cast Icon Tile returns one lowercase 16-character `xxh3_64_hexdigest`, while a ten-Icon Tile blacklist condition may return a list of zero to ten such hashes.
...
- `raw_value()` preserves capture facts: Cell RGB channels, Value Bar white-fill percentage, or Icon Tile hash. Cell reading trusts only `array[1:3, 1:3]` from the 4×4 region. A Value Bar trusts only `bar_pix_array[1:3, :]`, counts pixels exactly equal to `(255, 255, 255)`, and returns `100.0 * white_count / total_count` or `0.0` for an empty inner region. Icon Tile reading trusts only `array[1:7, 1:7]` from the 8×8 region; an all-black inner region yields `None`, otherwise it yields the lowercase 16-character `xxh3_64_hexdigest` of the contiguous inner array with seed zero. Multi-Icon Tile raw results always have `output_count` entries and preserve absent slots as `None`.
- `value()` performs plugin-specific calculation, scaling, filtering, and fallback and must always return a value matching the declared type and shape. A plugin may remove empty Icon Tile slots or define a different business order; the core does not prescribe list ordering.
...
- Deferred to later protocol or implementation tasks: capture channel-order normalization, concrete general-cell fields, exact Cell and Value Bar encodings, loop frequency, full UI behavior, generated addon display metadata, and talent-aware rotation routing.
...
- 2026-09-05 source check of the user-referenced M.I.D.N.I.G.H.T `12.0` `Terminal/terminal/pixelcalc/cell.py` confirmed 4×4 Cell inner cropping with `[1:3, 1:3]`, 8×8 Icon Tile inner cropping with `[1:7, 1:7]`, and seeded `xxh3_64_hexdigest` over a contiguous NumPy array.
- 2026-09-05 source check of the user-referenced M.I.D.N.I.G.H.T `12.0` `Terminal/terminal/pixelcalc/matrix.py` confirmed Value Bar cropping with `[1:3, :]`, exact white-pixel counting, and a `0.0`–`100.0` percentage result.
...
- 2026-09-05 source-path check: all local files cited for AuraContainer, Secret Values, restricted execution, pixel crops, Value Bar decoding, hashing, and macro binding exist; the cited historical implementations contain the documented operations.
...
- 2026-09-05: Interview round 8 settled the base-class template flow, normalized Value Bar raw values, trusted inner Cell/Icon Tile crops, fixed-length raw Icon Tile lists, catch-all decode fallback behavior, Python-compatible condition titles, and Python-style literals. Expression execution remains open pending the requested performance explanation.
...
- 2026-09-05: The user revised Value Bar decoding to use the middle two rows and exact white-pixel percentage, superseding the prior normalized `0.0`–`1.0` decision, and added mandatory Python type hints. Type-hint enforcement depth remains the only open affected branch.

...
需要分配像素的条件实例只能选择 `cell`、`value_bar`、`icon_tile` 中的一种，但可以连续占用多个同类区域。不新增区域时使用 `none`、数量为 0，冻结空布局；它不改变其他条件坐标或基板宽度。区域数量在插件参数通过校验后计算，并在布局完成时冻结；运行中不得改变。插件还可通过同帧 PixelDecoder 读取分配区域以外的有效坐标，见[插件系统](plugin-system.md#基类契约)。

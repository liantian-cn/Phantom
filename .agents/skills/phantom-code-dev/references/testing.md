...
- 宏自动键位分配：固定池共 148 个唯一键位、分组顺序与标点顺序、两种 ALT F4 排除、全部宏按声明顺序分配（包括未引用宏）、每份 rotation 独立从池开头分配；空宏列表与 148 个宏允许，149 个宏拒绝加载且不回写。
...
- 集合生成一次输出全部 UUID Lua 与共享文件；重复组合/UUID、空集合和后续项渲染失败在写产物前拒绝。TUI 生成复用启动快照，文件修改重启生效。
...
- 固定布局与优先级的引擎/TUI测试使用独立 fixture，不依赖正式职业循环的动作或条件数。40份正式辅助循环另做集合、顺序、转换、门控、参数、加载副本无回写和Lua语法验证，不依赖外部参考仓库才能执行。

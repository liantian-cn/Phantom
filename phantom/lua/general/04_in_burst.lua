--[[
original: general\04_in_burst.lua
uuid: 0db46515-244d-4755-9a77-65b3581d85ef
index: 4
摘要：在第一行第 4 个 Cell 中以黑白显示插件爆发状态。

描述：
    通过 UIInitFuncs 创建普通 Cell，沿用共享缩放和背景扩宽，初始保持黑色。
    独立 OnUpdate 使用随机初始延迟错峰刷新，累计时间超过 0.1 秒时扣除 0.1 秒并更新一次。
    每次读取当前爆发状态，true 显示白色，false 显示黑色。

修改记录：
2026-09-08：按通用爆发状态 Cell 需求新增。
]]
...
local random = math.random -- 生成初始随机延迟，使两个状态 Cell 错峰刷新
...
local inBurstCell -- 爆发状态 Cell，等待 UI 初始化后赋值
local eventFrame = CreateFrame("Frame") -- 独立承载本字段的 OnUpdate 刷新
local fastTimeElapsed = -random() -- 随机负初值推迟首次刷新，不改变全局随机种子

local function RefreshInBurstCell()
    if not inBurstCell then -- 更新回调可能先于延迟 UI 初始化到达
        return
    end

    inBurstCell:setCellBoolean(addonTable.InBurst()) -- 实时读取爆发状态，true 白色、false 黑色
end

local function InitializeInBurstCell()
    inBurstCell = Cell:New({ x = 4, y = 1 }) -- 构造后保持默认黑色，等待错峰首次刷新
end
...
insert(UIInitFuncs, InitializeInBurstCell) -- 沿用共享初始化顺序和缩放

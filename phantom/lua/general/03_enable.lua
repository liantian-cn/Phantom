--[[
original: general\03_enable.lua
uuid: c845da87-d22c-462b-9696-0678101df51b
index: 3
摘要：在第一行第 3 个 Cell 中以黑白显示插件启用状态。

描述：
    通过 UIInitFuncs 创建普通 Cell，沿用共享缩放和背景扩宽，初始保持黑色。
    独立 OnUpdate 使用随机初始延迟错峰刷新，累计时间超过 0.1 秒时扣除 0.1 秒并更新一次。
    每次读取当前启用状态，true 显示白色，false 显示黑色。

修改记录：
2026-09-08：按通用启用状态 Cell 需求新增。
]]
...
local enableCell -- 启用状态 Cell，等待 UI 初始化后赋值
...
local function RefreshEnableCell()
    if not enableCell then -- 更新回调可能先于延迟 UI 初始化到达
        return
    end

    enableCell:setCellBoolean(addonTable.ENABLE) -- 实时读取启用状态，true 白色、false 黑色
end

local function InitializeEnableCell()
    enableCell = Cell:New({ x = 3, y = 1 }) -- 构造后保持默认黑色，等待错峰首次刷新
end
...
insert(UIInitFuncs, InitializeEnableCell) -- 沿用共享初始化顺序和缩放

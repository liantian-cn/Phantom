"""
Summary:
    读取不可辅助目标首个匹配减益的原生剩余时长条百分比。
Description:
    aura_ids 必填且为非空正整数列表；固定 PLAYER|HARMFUL，PLAYER 包括玩家、宠物和载具。
    不接受 player_only、duration 或 width；单个 ValueBar 输出 float scalar，固定内容宽度 5 Cell（20×4 像素），含红色分隔占 6 Cell（24×4）。
    目标须存在且 UnitCanAssist("player", "target", true, true) 为假，不增加 UnitCanAttack 门控。
    ...
    无光环、目标无效、无有效黑白像素或解码异常返回 0.0；永久光环交官方处理，不保证满条，游戏渲染需另行验收。
Key Variables:
    aura_ids: 官方槽候选光环 ID 集合，不保证按配置列表顺序首匹配。
Change Log:
    2026-09-20: Added 固定 20 像素内容宽度的目标减益剩余时长百分比插件。
"""

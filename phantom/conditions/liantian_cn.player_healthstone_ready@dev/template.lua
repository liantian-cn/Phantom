--[[
original: ../conditions/liantian_cn.player_healthstone_ready@dev/template.lua
uuid: {{uuid}}
plugin: liantian_cn.player_healthstone_ready@dev
摘要：治疗石是否准备就绪。
描述：
...
--[[
用途与签名：start, duration, enabled = C_Item.GetItemCooldown(itemID)；返回冷却起点、时长和布尔启用标记。usable, noMana = C_Item.IsUsableItem(itemID)；返回可用及资源不足布尔值。
业务限制：固定物品 224464；冷却为零且启用、可用、不缺资源。不另查背包数量。
...
旧项目参考：PhantomProject/src/0117_player_healthstone_ready.lua；revision f6935113e686eb73785b8315c58368093012c959。

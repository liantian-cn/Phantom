--[[
original: ../conditions/spell_in_range@dev/template.lua
uuid: {{uuid}}
摘要：显示指定技能对配置单位的射程结果。
描述：只接受 target/focus/mouseover；0.1 秒随机错峰轮询，不注册目标切换事件。
      潜在秘密结果直接交布尔颜色消费者；普通 nil、无单位、无效技能显示否。
修改记录：
2026-09-18：新增通用射程条件。
]]
...
local UNIT_TOKEN = {{unit_token}} -- 已验证单位白名单

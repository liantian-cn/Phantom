"""
Summary:
    从当前帧的通用 Cell 读取Lua 延迟状态。
Description:
    参数：无参数；不包含 Lua，不分配新像素区域。
    输出：none，0 个区域，bool scalar；读取已有 Cell(5, 1) 的 4×4 像素。
    来源：phantom/lua/general/05_delaying.lua，现有 Lua 框架持续维护该状态。
...
Change Log:
    ...
    2026-09-14: Added 按确认计划将正在延迟读取改为可命名的 Python 条件插件。
"""

...
uuid: {{uuid}}
摘要：显示玩家整灵魂碎片数量。
描述：固定 SoulShards，非原始单位返回整碎片；秘密值、越界或非整数直接报错。
...
local UnitPower = UnitPower -- 读取整碎片，不读取原始碎片片段
...
固定 player、SoulShards、false，保留整碎片单位；API 标记 SecretWhenUnitPowerRestricted。

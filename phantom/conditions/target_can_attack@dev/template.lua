--[[
original: ../conditions/target_can_attack@dev/template.lua
uuid: {{uuid}}
摘要：目标是否可攻击。
...
local UnitCanAttack = UnitCanAttack -- 本条件业务状态
local EvaluateColorFromBoolean = C_CurveUtil.EvaluateColorFromBoolean -- 安全颜色消费者
--[[
UnitCanAttack("player", unit) 返回玩家能否攻击该单位的 bool；不等同敌对或不可辅助。
...
Wiki：https://warcraft.wiki.gg/wiki/API_UnitCanAttack （2026-09-18 获取；网页版本不替代目标 build 源码）。
]]
--[[  variable reference  ]]
...
local function update()
    ...
    display:setCellBoolean(UnitCanAttack("player", UNIT_TOKEN))
end

--[[
original: ../conditions/target_is_exists@dev/template.lua
uuid: {{uuid}}
摘要：目标是否存在。
...
--[[
UnitExists(unit) 返回 bool；普通 target/focus token 的存在查询用于前置检查。
...
Wiki：https://warcraft.wiki.gg/wiki/API_UnitExists （2026-09-18 获取；网页版本不替代目标 build 源码）。
]]
--[[  variable reference  ]]

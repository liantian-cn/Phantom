--[[
original: ../conditions/liantian_cn.player_melee_enemies_count@dev/template.lua
uuid: {{uuid}}
plugin: liantian_cn.player_melee_enemies_count@dev
摘要：玩家近战范围敌人数量。
描述：
...
local function update()
    ...
    for index = 1, NAMEPLATE_LIMIT do
        ...
        if UnitExists(unit) and UnitCanAttack(UNIT_TOKEN, unit) then
            local inRange = IsSpellInRange(SPELL_ID, unit)
            if not issecretvalue(inRange) and inRange ~= nil and inRange then
                count = count + 1
            end
        end
    end

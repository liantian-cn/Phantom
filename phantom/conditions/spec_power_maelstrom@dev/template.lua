--[[
original: ../conditions/spec_power_maelstrom@dev/template.lua
uuid: {{uuid}}
摘要：显示玩家漩涡值比例。
描述：固定 Maelstrom，秘密资源只经黑白曲线交给 Cell；初始化及资源事件刷新。
...
local POWER_TYPE = Enum.PowerType.Maelstrom -- 固定漩涡值资源
local insert = table.insert -- 注册初始化
--[[
UnitPowerPercent(unitToken, powerType, unmodified, curve) 返回曲线求值结果。
固定 player、Maelstrom、false；SecretWhenUnitPowerRestricted/SecretWhenCurveSecret，

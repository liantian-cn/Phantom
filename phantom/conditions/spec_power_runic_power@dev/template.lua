--[[
original: ../conditions/spec_power_runic_power@dev/template.lua
uuid: {{uuid}}
摘要：显示玩家符文能量比例。
描述：固定 RunicPower，秘密资源只经黑白曲线交给 Cell；初始化及资源事件刷新。
...
local POWER_TYPE = Enum.PowerType.RunicPower -- 固定符文能量资源
local insert = table.insert -- 注册初始化
--[[
UnitPowerPercent(unitToken, powerType, unmodified, curve) 返回曲线求值结果。
固定 player、RunicPower、false；SecretWhenUnitPowerRestricted/SecretWhenCurveSecret，

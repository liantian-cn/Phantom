--[[
original: ../conditions/target_is_alive@dev/template.lua
uuid: {{uuid}}
摘要：目标是否存活。
...
local UnitIsDeadOrGhost = UnitIsDeadOrGhost -- 本条件业务状态
local EvaluateColorFromBoolean = C_CurveUtil.EvaluateColorFromBoolean -- 安全颜色消费者
--[[
UnitIsDeadOrGhost(unit) 返回死亡或灵魂 bool；存在前置后经颜色消费者反转，不在 Lua 中取反。
...
Wiki：https://warcraft.wiki.gg/wiki/API_UnitIsDeadOrGhost （2026-09-18 获取；网页版本不替代目标 build 源码）。
]]
--[[  variable reference  ]]
...
local function update()
    ...
    display:setCell(EvaluateColorFromBoolean(UnitIsDeadOrGhost(UNIT_TOKEN), COLOR.BLACK, COLOR.WHITE))
end

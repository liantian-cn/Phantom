"""
Summary:
    从当前帧的通用 Cell 读取Lua 控制条的启用状态。
Description:
    参数：无参数；不包含 Lua，不分配新像素区域。
    输出：none，0 个区域，bool scalar；读取已有 Cell(3, 1) 的 4×4 像素。
    来源：phantom/lua/general/03_enable.lua，现有 Lua 框架持续维护该状态。
    不调用 WoW API、不处理 Secret Value，只读取框架已经渲染的像素。
    解码：中心 2×2 严格全白为 True、全黑为 False。
    非黑白值或额外读取异常兜底 True，rotation 继续按配置求值，不隐式暂停。
Key Variables:

...
Description:
    ...
    API：UnitCastingInfo("target") 第 11 项 delayTimeMs、UnitChannelInfo("target") 第 9 项 isEmpowered 是 NeverSecret 状态哨兵；UnitCastingDuration/UnitChannelDuration("target") 返回 duration 或无值。color = duration:EvaluateElapsedPercent(curve) 接受颜色曲线并返回可能秘密的颜色。
...
Key Variables:
    None
Change Log:
    2026-09-18: Added 按已确认的目标条件迁移计划新增 target_cast_progress@dev。
"""

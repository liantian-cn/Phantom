--[[
original: ../conditions/target_cast_progress@dev/template.lua
uuid: {{uuid}}
...
修改记录：
2026-09-18：事件统一延至下一帧刷新；统一 0.1 秒轮询写法并使用 UPDATE_INTERVAL。
2026-09-18：按已确认计划新增目标条件插件。
]]
...
--[[
用途与签名：UnitCastingInfo("target") 第 11 项 delayTimeMs、UnitChannelInfo("target") 第 9 项 isEmpowered 是 NeverSecret 状态哨兵；UnitCastingDuration/UnitChannelDuration("target") 返回 duration 或无值。color = duration:EvaluateElapsedPercent(curve) 接受颜色曲线并返回可能秘密的颜色。
...
local UNIT_TOKEN = "target" -- 本版本固定目标单位
...
local function update()
    if not display then return end
    if not UnitExists(UNIT_TOKEN) then progressMode = nil; display:clearCell(); return end

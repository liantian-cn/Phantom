...
Description:
    ...
    仅在有施法／引导时将 notInterruptible 交给 EvaluateColorFromBoolean 反向着色；普通 nil 为 False。
...
Key Variables:
    None
Change Log:
    2026-09-18: Added 按已确认的目标条件迁移计划新增 target_cast_interruptible@dev。
"""
...
class Plugin(Condition):
    """目标当前施法或引导是否可中断；参数、像素解码及异常兜底均属于本插件。"""
